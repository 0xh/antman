...
exports.verify = function (jwtToken) {
  return new Promise((resolve, reject) => {
    ...
    jwt.verify(jwtToken, secretKey, options, (error, decoded) => {
      ...
      if (error && (error.constructor.name === 'TokenExpiredError' || error.constructor.name === 'JsonWebTokenError')) {
...
exports.grant = function (user) {
  ...
  const payload = {
    user: userId,
    exp: moment().utc().add(config.api.accessTokenTtl, 'seconds').unix(), // the number of seconds since the Unix Epoch
  }
    ...
    jwt.sign(payload, secretKey, options, (err, token) => {
      if (err) {
        ...
        resolve({
          accessToken: token,
          tokenType: 'bearer',
          expiresIn: moment().utc().add(config.api.accessTokenTtl, 'seconds').toISOString(),
          refreshToken,
        })
      }
    })
